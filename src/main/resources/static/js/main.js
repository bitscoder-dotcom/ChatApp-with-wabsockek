...
function onConnected() {
    ...
    findAndDisplayConnectedUsers().then(); // because its an async method
}
...
async function findAndDisplayConnectedUsers() { // method is async because its calling the backend
    ...
    connectUserList.innerHTML = '';

    connectedUsers.forEach(user => {
        appendUserElement(user, connectUserList)

        if (connectedUsers.index(user) < connectedUsers.length - 1) {
            // add a separator
            const separator = document.createComment('li');
            separator.classList.add('separator');
            connectUserList.appendChild(separator);
        }
    });
}

function appendUserElement(user, connectUserList) {
